...
/// Only sidebar tiles are tapped: labels like "Yojna" also appear in the top
/// bar and in tables.
...
void main() {
  for (final mode in [ThemeMode.light, ThemeMode.dark]) {
    for (final entry in _widths.entries) {
      testWidgets(
          'every page lays out cleanly on ${entry.key} (${mode.name})',
          (tester) async {
        await _pumpApp(tester, entry.value, themeMode: mode);
        expect(
        ...
        for (final item in AppRoutes.nav) {
          ...
          expect(
            ...
            isNull,
            reason: '${item.label} overflowed at ${entry.key} (${mode.name})',
          );
        }
      });
    }
  ...
  testWidgets('add-member dialog opens and validates', (tester) async {
    ...
    await tester.pumpAndSettle();
    await tester.tap(find.text(S.addMember).last);
    await tester.pumpAndSettle();
  ...
  testWidgets('scheme scope filters the member list', (tester) async {
    ...
    final all = find.textContaining('members', findRichText: false);
